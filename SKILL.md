...
2. `normalized_dose_pct` 컬럼: 이 변수는 VIF > 1000의 극심한 다중공선성을 가지므로 **학습 피처에서 제거**합니다.
3. `exposure_dose_mj_cm2` 등 고VIF 변수는 트리 모델(XGBoost)에는 영향이 적으므로 **유지**합니다.
...
**잘못된 방법:** 해당 컬럼을 피처 목록에 포함한 채 학습  
**증상:** VIF > 1000의 극심한 다중공선성으로 모델 해석 불가, 변수 중요도 왜곡  
**올바른 방법:** `num_features` 목록에서 완전히 제거. (XGBoost는 자체적으로 강건하지만 이 변수는 제거가 원칙)

### 3. NEGATIVE PR의 무분별한 모델 튜닝 시도
